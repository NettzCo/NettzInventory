...
import { obtenerConteoAlertas, obtenerConteoChatNoLeido } from "./alertas/liveActions";
...
export default function Sidebar({
  ...
  const [chatCount, setChatCount] = useState(chatCountInicial);
  ...
  useEffect(() => {
    setChatCount(chatCountInicial);
  }, [chatCountInicial]);

  // Escucha en tiempo real, con la campanita de Alertas y el globito de Chat
  // separados por completo: un mensaje de chat nuevo (o marcarlo como
  // leído) solo mueve el número de Chat — nunca el de Alertas. Pedidos,
  // vencimientos y cambios de estado de SIM sí siguen moviendo Alertas.
  useEffect(() => {
    ...
    async function refrescarChat() {
      const nuevo = await obtenerConteoChatNoLeido();
      setChatCount(nuevo);
    }
          ...
          {items.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.href === "/dashboard/chat" && chatCount > 0 && (
                  <span
                    className="text-xs font-semibold rounded-full px-2 py-0.5"
                    style={{ background: "var(--chip-gold)", color: "var(--ink-950)" }}
                  >
