...
import { formatFecha, formatPlan, estadoEfectivo } from "@/lib/ui";
...
export default async function DashboardPage({
  ...
  // Antes esto escaneaba sim_cards.select("proveedor") sin límite — pero
  // PostgREST corta en silencio cualquier select sin .range() a 1000 filas,
  // y como se ordenaba alfabéticamente, con miles de SIMs de Claro las
  // primeras 1000 filas eran TODAS Claro: los demás proveedores (Moabits,
  // Wireless Logic, etc.) nunca aparecían en el filtro. La tabla
  // "providers" no tiene ese problema — es la lista de proveedores
  // configurados, sin importar cuántas SIMs tenga cada uno.
  const { data: proveedoresRows } = await supabase
    .from("providers")
    .select("name")
    .eq("active", true)
    .order("name");
  const proveedores = (proveedoresRows ?? []).map((p) => p.name);
  ...
  const { data: clientesRows } = await supabase
    ...
    .eq("active", true)
    .order("nombre");
  const clientesOpciones = clientesRows ?? [];
    ...
    <main className="p-8">
      ...
      <form className="flex flex-wrap items-end gap-3 mb-5" method="get">
        ...
        <FilterField label="Cliente">
          <ClienteFilterInput name="cliente" defaultValue={cliente ?? ""} clientes={clientesOpciones} placeholder="Buscar cliente…" className="input-filter" />
        </FilterField>
              ...
              >
                ...
                <td className="px-4 py-3">
                  <Link href={`/dashboard/sim/${sim.id}`}>
                    <StatusPill estado={estadoEfectivo(sim)} />
                  </Link>
                </td>
                <Td><Link href={`/dashboard/sim/${sim.id}`} className="block">{formatFecha(sim.estado_desde)}</Link></Td>
                <Td>
                  <Link href={`/dashboard/sim/${sim.id}`} className="block">
                    {sim.estado_actual === "Activa" && sim.tipo_plan === "Prepago" && sim.fecha_entrega
                      ? formatFecha(addMonths(new Date(`${sim.fecha_entrega}T00:00:00`), sim.duracion_meses ?? 12).toISOString())
                      : "—"}
                  </Link>
                </Td>
              </tr>
