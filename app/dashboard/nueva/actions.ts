...
export async function confirmarCargaMasiva(filas: FilaParseada[]) {
  ...
  for (const f of filas) {
    ...
    if (!existente) {
      ...
      const simId = simCard.id as string;
      await supabase.from("sim_status_history").insert({ sim_id: simId, estado: f.estado, changed_by: userId, nota: "Estado inicial (carga masiva).", bulk_operation_id: operacionId });
      if (f.numeroCorto) {
