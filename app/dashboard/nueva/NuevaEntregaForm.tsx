...
function nuevaFila(): SimRowInput {
  return { icc: "", proveedor: "", numero_corto: "", estado_entrega: "Inactiva", fecha_activacion: new Date().toISOString().slice(0, 10), apn: "", observaciones: "" };
}
                ...
                {sim.estado_entrega === "Activa" && (
                  <Field label="Fecha de activación" required>
                    <input
                      required
                      type="date"
                      value={sim.fecha_activacion}
                      onChange={(e) => updateSim(i, { fecha_activacion: e.target.value })}
                      className="input"
                    />
                    <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
                      Puede ser una fecha pasada (por ejemplo, al cargar inventario ya activado antes).
                      {tipoPlan === "Prepago" && sim.fecha_activacion && (
                        <> Con {duracionMeses} meses, el plan vence el <strong>{sumarMeses(sim.fecha_activacion, Number(duracionMeses))}</strong>.</>
                      )}
                    </p>
                  </Field>
                )}
