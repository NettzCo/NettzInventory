...
// Tamaño de cada tanda que el navegador manda al confirmar — chico a
// propósito para que la llamada nunca se acerque a ningún límite de tamaño
// del servidor, sin importar cuánto pese cada fila ni qué límite tenga
// configurado el servidor en un momento dado.
const TAMANO_TANDA_CONFIRMACION = 300;
...
export default function CargaMasivaForm() {
  ...
  function resolverNumeroCorto(fila: number, valor: "reasignar" | "no_cargar") {
    setFilas((prev) => prev.map((f) => (f.fila === fila ? { ...f, resolucionNumeroCorto: valor } : f)));
  }

  function handleConfirmar() {
    setProgreso({ hechas: 0, total: filas.length });
    startTransition(async () => {
      const acumulado = { creadas: 0, actualizadas: 0, sinCambios: 0, resultados: [] as FilaResultado[] };

      for (let i = 0; i < filas.length; i += TAMANO_TANDA_CONFIRMACION) {
        const tanda = filas.slice(i, i + TAMANO_TANDA_CONFIRMACION);
        try {
          const res = await confirmarCargaMasiva(tanda);
          acumulado.creadas += res.creadas ?? 0;
          acumulado.actualizadas += res.actualizadas ?? 0;
          acumulado.sinCambios += res.sinCambios ?? 0;
          acumulado.resultados.push(...(res.resultados ?? []));
          if (res.error) {
            // Un error a nivel de toda la tanda (no de una fila puntual) —
            // se registra para cada fila de esa tanda y se sigue con las
            // siguientes, para no perder el resto de la carga por una tanda.
            for (const f of tanda) acumulado.resultados.push({ fila: f.fila, icc: f.icc, ok: false, error: res.error });
          }
        } catch (e) {
          const mensaje = e instanceof Error ? e.message : "Error desconocido al confirmar esta tanda.";
          for (const f of tanda) acumulado.resultados.push({ fila: f.fila, icc: f.icc, ok: false, error: mensaje });
        }
        setProgreso({ hechas: Math.min(i + TAMANO_TANDA_CONFIRMACION, filas.length), total: filas.length });
      }
  ...
  const filasConConflictoNumero = filas.filter((f) => f.errores.length === 0 && f.numeroCortoConflictoIcc);
    ...
    <section className="rounded-xl border bg-white p-6" style={{ borderColor: "var(--border)" }}>
      ...
      {fase === "revisar" && (
        <div>
          ...
          {filasConConflictoNumero.length > 0 && (
            <div className="mb-5">
              <h3 className="text-sm font-semibold mb-2" style={{ color: "var(--state-desactivada-temp)" }}>
                Estos números cortos ya están en otra SIM
              </h3>
              <div className="flex flex-col gap-3">
                {filasConConflictoNumero.map((f) => (
                  <div key={f.fila} className="rounded-lg border p-3" style={{ borderColor: "var(--state-desactivada-temp)", background: "#FFF6EE" }}>
                    <p className="text-sm mb-2">
                      Fila {f.fila} (ICC <span className="font-mono">{f.icc}</span>): el número corto{" "}
                      <span className="font-mono font-medium">{f.numeroCorto}</span> ya está asignado a la SIM{" "}
                      <span className="font-mono font-medium">{f.numeroCortoConflictoIcc}</span>, que está{" "}
                      <strong>Desactivada</strong> — por eso se puede liberar y reasignar.
                    </p>
                    <div className="flex flex-col gap-1.5">
                      <label className="flex items-center gap-1.5 text-sm">
                        <input
                          type="radio"
                          name={`numero-${f.fila}`}
                          checked={f.resolucionNumeroCorto === "reasignar"}
                          onChange={() => resolverNumeroCorto(f.fila, "reasignar")}
                        />
                        Sí, liberar el número de la SIM {f.numeroCortoConflictoIcc} y asignarlo a la SIM {f.icc}
                      </label>
                      <label className="flex items-center gap-1.5 text-sm">
                        <input
                          type="radio"
                          name={`numero-${f.fila}`}
                          checked={f.resolucionNumeroCorto === "no_cargar"}
                          onChange={() => resolverNumeroCorto(f.fila, "no_cargar")}
                        />
                        No, dejar el número donde está — esta fila se carga sin número corto
                      </label>
                    </div>
                  </div>
                ))}
            ...
            <div className="mb-4">
              <div className="h-2 rounded-full overflow-hidden" style={{ background: "var(--border)" }}>
                <div
                  ...
                  style={{ width: `${Math.round((progreso.hechas / progreso.total) * 100)}%`, background: "var(--ink-900)" }}
                />
              </div>
              <p className="text-xs mt-1.5" style={{ color: "var(--text-muted)" }}>
                Guardando {progreso.hechas} de {progreso.total} — se manda en tandas chicas para que nunca falle por tamaño, esto puede tardar un poco con archivos grandes.
              </p>
            </div>
          )}
          ...
          <div className="flex gap-3">
            ...
            >
              {isPending
                ? progreso
                  ? `Guardando… (${progreso.hechas}/${progreso.total})`
                  : "Guardando…"
                : filasExistentes.length > 0
      ...
      {fase === "listo" && resultado && (
        <div>
          {(() => {
            const huboExito = resultado.creadas + resultado.actualizadas + resultado.sinCambios > 0;
            const todoFallo = errores.length > 0 && !huboExito;
            const estilo = todoFallo
              ? { background: "#FDEAEA", color: "var(--state-desactivada)" }
              : errores.length > 0
                ? { background: "#FDF3E4", color: "var(--state-lista)" }
                : { background: "#E7F5EC", color: "var(--state-activa)" };
            return (
              <p className="text-sm rounded-lg px-3 py-2 mb-3 font-medium" style={estilo}>
                {todoFallo
                  ? `No se cargó ninguna SIM — ${errores.length} con error. Revisa el detalle abajo.`
                  : <>
                ...
                <tbody>
                  {errores.map((e) => (
                    <tr key={e.fila} className="border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                      ...
                      <td className="px-3 py-2" style={{ color: "var(--state-desactivada)" }}>{traducirError(e.error)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
