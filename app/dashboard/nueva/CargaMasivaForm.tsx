      ...
      {fase === "revisar" && (
        <div>
          ...
          {filasExistentes.length > 0 && (
            <div className="rounded-lg px-3 py-2.5 mb-5 text-sm" style={{ background: "var(--chip-gold-soft)", color: "var(--text-primary)" }}>
              <p className="font-medium mb-1">
                {filasExistentes.length} de estos ICC ya están en el inventario — se van a actualizar, no a duplicar.
              </p>
              <p style={{ color: "var(--text-secondary)" }}>
                El cambio queda registrado en la hoja de vida de cada SIM. ICC: {filasExistentes.map((f) => f.icc).join(", ")}.
              </p>
            </div>
          )}
