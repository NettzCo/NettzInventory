    ...
    startTransition(async () => {
      const res = await aplicarCambios(
        entrada,
        {
          ...
          fechaActivacion: cambiarEstado && nuevoEstado === "Activa" ? fechaActivacion : undefined,
          cambiarAsignacion,
        ...
        <div className="rounded-lg border p-4" style={{ borderColor: "var(--border)" }}>
          ...
          {cambiarEstado && (
            <>
              ...
              {nuevoEstado === "Activa" && (
                <div className="mt-3">
                  <label className="text-xs font-medium block mb-1" style={{ color: "var(--text-secondary)" }}>Fecha de activación</label>
                  <input type="date" className="input" value={fechaActivacion} onChange={(e) => setFechaActivacion(e.target.value)} />
                  <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
                    Puede ser una fecha pasada (por ejemplo, al cargar inventario que ya estaba activado antes).
