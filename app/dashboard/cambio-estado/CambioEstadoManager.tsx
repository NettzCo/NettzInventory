...
import { aplicarCambios, revertirOperacionMasiva, consultarEstadoActual, ResultadoCambioMasivo, AsignacionRapida } from "./actions";
...
// Suma meses a una fecha (YYYY-MM-DD), solo para la vista previa de
// "fecha fin del plan" — sin depender de librerías externas en el cliente.
...
function FormularioCambio({
  ...
  const [cambiarFechaActivacion, setCambiarFechaActivacion] = useState(false);
  const [fechaActivacion, setFechaActivacion] = useState(() => new Date().toISOString().slice(0, 10));
  ...
  // Al escribir un ICC en modo individual, se consulta su estado actual
  // para precargarlo por defecto en el selector — en vez de arrancar
  // siempre en "Activa".
  useEffect(() => {
    if (modo !== "individual" || !icc.trim()) {
      setEstadoActual(null);
      return;
    }
    let vigente = true;
    const iccConsultado = icc.trim();
    const timeout = setTimeout(async () => {
      const res = await consultarEstadoActual(iccConsultado);
      if (!vigente) return;
      if ("estado" in res && res.estado) {
        setEstadoActual(res.estado);
  ...
  function aplicar(e: React.FormEvent) {
    ...
    setError(null);
    setResultado(null);
    if (!cambiarEstado && !cambiarFechaActivacion && !cambiarAsignacion) {
      setError("Elige al menos algo para cambiar: el estado, la fecha de activación, o el cliente/plan/precio.");
      return;
    }
    ...
    startTransition(async () => {
      const res = await aplicarCambios(
        entrada,
        {
          ...
          cambiarFechaActivacion,
          fechaActivacion: cambiarFechaActivacion ? fechaActivacion : undefined,
          cambiarAsignacion,
      ...
      setIccsTexto("");
      setNota("");
      setCambiarFechaActivacion(false);
  ...
  return (
    <section className="rounded-xl border bg-white p-6" style={{ borderColor: "var(--border)" }}>
      ...
      <form onSubmit={aplicar} className="flex flex-col gap-4 max-w-xl">
              ...
              <select className="input" value={nuevoEstado} onChange={(e) => setNuevoEstado(e.target.value)}>
                {ESTADOS_SIM.map((e) => (
                  <option key={e} value={e}>
                    Nuevo estado: {e}{estadoActual === e ? " (estado actual)" : ""}
                  </option>
                ))}
              </select>
              <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
                {estadoActual
                  ? "Se precargó con el estado actual de la SIM. Cámbialo solo si vas a modificarlo."
                  : modo === "individual"
                    ? "Escribe el ICC para ver el estado actual."
                    : "En carga masiva, cada SIM conserva su propio estado anterior en el historial."}
              </p>
            </>
          )}
        </div>

        {/* Sección 2: fecha de activación — independiente del cambio de estado */}
        <div className="rounded-lg border p-4" style={{ borderColor: "var(--border)" }}>
          <label className="flex items-center gap-2 text-sm font-medium mb-3">
            <input type="checkbox" checked={cambiarFechaActivacion} onChange={(e) => setCambiarFechaActivacion(e.target.checked)} />
            Cambiar la fecha de activación
          </label>
          {cambiarFechaActivacion && (
            <div>
              <input type="date" className="input" value={fechaActivacion} onChange={(e) => setFechaActivacion(e.target.value)} />
              <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
                Independiente del estado: puede ser una fecha pasada (por ejemplo, al cargar inventario que ya estaba activado antes), y se puede corregir sin necesidad de cambiar el estado.
                {cambiarAsignacion && asignacion.tipo_plan === "Prepago" && fechaActivacion && (
                  <> Con {asignacion.duracion_meses} meses, el plan vence el <strong>{sumarMeses(fechaActivacion, Number(asignacion.duracion_meses))}</strong>.</>
                )}
              </p>
            </div>
          )}
        ...
        {/* Sección 3: cliente, plan y precio */}
        <div className="rounded-lg border p-4" style={{ borderColor: "var(--border)" }}>
          <label className="flex items-center gap-2 text-sm font-medium mb-3">
          ...
          {cambiarAsignacion && (
            <div className="flex flex-col gap-3">
              ...
              <ClienteAutocomplete
                ...
                onChange={(v) => patch({ cliente_nombre: v })}
                clientes={clientes.map((c) => ({ id: c.id, nombre: c.nombre ?? c.full_name ?? "" }))}
      ...
      {resultado?.avisos && resultado.avisos.length > 0 && (
        <div className="text-xs mt-2" style={{ color: "var(--state-sin-numero)" }}>
          <p className="font-medium mb-1">{resultado.avisos.length} ICC quedaron distinto a lo pedido:</p>
          <ul className="list-disc pl-4 max-h-32 overflow-y-auto">
            {resultado.avisos.map((a, i) => (
              <li key={i}>{a.icc} — {a.motivo}</li>
            ))}
          </ul>
        </div>
      )}
