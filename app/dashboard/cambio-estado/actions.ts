  ...
  try {
    ...
    for (const icc of iccs) {
      ...
      if (!opciones.cambiarAsignacion && opciones.cambiarEstado) {
        ...
        // Si el destino es "Activa" y se indicó una fecha de activación,
        // se permite aplicar aunque ya estuviera en ese estado — la
        // intención es corregir/registrar la fecha, no "cambiar" de estado.
        const soloCorrigiendoFecha = opciones.nuevoEstado === "Activa" && !!opciones.fechaActivacion;
        if (sim.estado_actual === opciones.nuevoEstado && !soloCorrigiendoFecha) {
