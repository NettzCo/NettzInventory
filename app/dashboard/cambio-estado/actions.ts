...
import { resolverEstadoAGuardar } from "@/lib/estadoSinNumero";
...
async function buscarSimsPorIcc(
  ...
  const { data: sims } = await supabase
    ...
    .select("id, icc, estado_actual, cliente_actual, proveedor, numero_corto_actual")
  ...
  for (const s of sims ?? []) {
    mapa.set(s.icc, {
      ...
      numero_corto_actual: s.numero_corto_actual,
    });
  }
  return mapa;
}

export async function consultarEstadoActual(icc: string): Promise<{ estado: EstadoSim | null } | { error: string }> {
  try {
    ...
    if (!iccLimpio) return { estado: null };
    ...
    return { estado: (data?.estado_actual as EstadoSim | undefined) ?? null };
  } catch (e) {
    return { error: e instanceof Error ? e.message : "No se pudo consultar el estado actual." };
  }
}
...
export interface ResultadoCambioMasivo {
  ...
  avisos?: { icc: string; motivo: string }[];
}
...
export interface OpcionesCambio {
  ...
  nuevoEstado?: string;
  cambiarFechaActivacion: boolean; // independiente del cambio de estado — corrige/registra la fecha
  fechaActivacion?: string; // puede ser una fecha pasada
...
export async function aplicarCambios(iccsTexto: string, opciones: OpcionesCambio, nota: string): Promise<ResultadoCambioMasivo> {
  try {
    ...
    if (!opciones.cambiarEstado && !opciones.cambiarFechaActivacion && !opciones.cambiarAsignacion) {
      return { error: "Elige al menos algo para cambiar: el estado, la fecha de activación, o el cliente/plan/precio." };
    }
    ...
    if (opciones.cambiarFechaActivacion && !opciones.fechaActivacion) {
      return { error: "Indica la fecha de activación." };
    }
    ...
    // Se registra un movimiento en el historial cuando se cambia el estado,
    // o cuando solo se corrige/registra la fecha de activación (sin cambiar
    // el estado en sí — quedan ligados en la base de datos porque cada
    // movimiento necesita un estado, pero la persona no tiene que tocar el
    // estado para corregir la fecha).
    const debeRegistrarHistorial = opciones.cambiarEstado || opciones.cambiarFechaActivacion;
    ...
    for (const icc of iccs) {
      ...
      // cambiando el estado/fecha (sin tocar cliente/plan) — si también se
      // va a asignar cliente/plan, la SIM queda válida de todas formas.
      if (!opciones.cambiarAsignacion && debeRegistrarHistorial) {
        if (!sim.entregada) {
          omitidas.push({ icc, motivo: "Todavía no está entregada a ningún cliente" });
          continue;
        }
        const estadoDestino = opciones.cambiarEstado ? opciones.nuevoEstado : sim.estado_actual;
        // Si solo se está corrigiendo la fecha (sin cambiar de estado), se
        // permite aplicar aunque el estado quede igual — la intención es
        // corregir/registrar la fecha, no "cambiar" de estado.
        const soloCorrigiendoFecha = opciones.cambiarFechaActivacion && !opciones.cambiarEstado;
        if (sim.estado_actual === estadoDestino && !soloCorrigiendoFecha) {
    ...
    if (opciones.cambiarFechaActivacion) partesNota.push(`fecha de activación → ${opciones.fechaActivacion}`);
    ...
    if (debeRegistrarHistorial) {
      const avisos: { icc: string; motivo: string }[] = [];
      const filas = aplicables.map((sim) => {
        const estadoDeseado = opciones.cambiarEstado ? opciones.nuevoEstado! : (sim.estado_actual as string);
        const estadoReal = resolverEstadoAGuardar({ estadoDeseado, proveedor: sim.proveedor, tieneNumeroCorto: !!sim.numero_corto_actual });
        if (estadoReal !== estadoDeseado) {
          avisos.push({ icc: sim.icc, motivo: `Se guardó como "${estadoReal}" en vez de "Activa": todavía no tiene número corto asignado.` });
        }
        return {
          sim_id: sim.id,
          estado: estadoReal,
          ...
          changed_by: userId,
          nota: nota.trim() || null,
          ...(opciones.cambiarFechaActivacion && opciones.fechaActivacion
      ...
      return { ok: true, aplicadas: aplicables.length, omitidas, avisos: avisos.length > 0 ? avisos : undefined };
    }
