    ...
    <tr className="border-b last:border-0 align-top" style={{ borderColor: "var(--border)" }}>
        ...
        {editing ? (
          <div className="flex gap-2 items-center">
            <input
              ...
              autoFocus
            />
            <button onClick={saveName} disabled={isPending} className="text-xs font-medium" style={{ color: "var(--state-activa)" }}>Guardar</button>
            <button onClick={() => { setEditing(false); setFullName(usuario.full_name); }} className="text-xs" style={{ color: "var(--text-muted)" }}>Cancelar</button>
          </div>
        ) : (
          <div className="flex items-center gap-2">
            <span>{fullName}</span>
            <button onClick={() => setEditing(true)} className="text-xs" style={{ color: "var(--text-secondary)" }}>Editar</button>
          </div>
        )}
        {feedback && (
      ...
      <td className="px-6 py-3">
        <div className="flex flex-col gap-1.5 items-start">
          <button onClick={() => setModulosAbierto(!modulosAbierto)} disabled={isPending} className="text-xs font-medium" style={{ color: "var(--text-secondary)" }}>
            {rolActual?.is_system ? "Acceso total" : `${modulosAbierto ? "Ocultar" : "Editar"} módulos`}
          </button>
          <button onClick={handleResetPassword} disabled={isPending} className="text-xs font-medium" style={{ color: "var(--text-secondary)" }}>
            Restablecer contraseña
          </button>
          <button onClick={handleDelete} disabled={isPending} className="text-xs font-medium" style={{ color: "var(--state-desactivada)" }}>
            Eliminar usuario
          </button>
        </div>
      </td>
