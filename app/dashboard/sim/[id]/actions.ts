...
export async function asignarNumeroCorto(simId: string, numeroCorto: string) {
  ...
  // Si ese número corto hoy es de OTRA SIM, solo se puede tomar si esa otra
  // SIM está "Desactivada" — si está Activa (o cualquier otro estado), se
  // bloquea por completo, sin excepción.
  const conflicto = await verificarConflictoNumeroCorto(supabase, profile.organization_id, numero, simId);
  if (!conflicto.puedeAsignar) {
    return { error: conflicto.mensaje ?? "Ese número corto ya está en uso por otra SIM que no está Desactivada." };
  }
  if (conflicto.simIdActual && conflicto.simIdActual !== simId) {
    const { error: errorLiberar } = await liberarNumeroCortoDeSim(supabase, conflicto.simIdActual, numero);
    if (errorLiberar) return { error: errorLiberar };
  }
  ...
  revalidatePath(`/dashboard/sim/${simId}`);
  return { ok: true, aviso: conflicto.mensaje ?? null };
}
