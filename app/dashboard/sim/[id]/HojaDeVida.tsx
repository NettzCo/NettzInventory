...
import { formatFecha, formatFechaHora, formatMoneda, estadoEfectivo, sumarMeses } from "@/lib/ui";
import { cambiarEstado, asignarNumeroCorto, consultarNumeroCorto, reasignarCliente, actualizarDetalles, renovarSim, desactivarSim, ReasignacionInput } from "./actions";
import ClienteAutocomplete, { ClienteOpcion } from "@/components/ClienteAutocomplete";
import { traducirError } from "@/lib/friendlyError";

type Panel = null | "estado" | "numero" | "cliente" | "detalles" | "renovar";
...
export default function HojaDeVida({
  ...
  // Solo tiene sentido mostrar un vencimiento si es Prepago — se cuenta
  // desde la fecha de entrega (la plantilla de carga masiva no siempre trae
  // una fecha de activación separada) más la duración del plan. Se calcula
  // con aritmética pura de calendario (sumarMeses), nunca con un objeto
  // Date con hora — así nunca se corre un día según la zona horaria de
  // quien esté mirando la página.
  const fechaVencimiento =
    current?.tipo_plan === "Prepago" && current?.fecha_entrega
      ? sumarMeses(current.fecha_entrega, current.duracion_meses ?? 12)
      : null;
  ...
  return (
    <main className="p-8 max-w-5xl">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <StatusPill estado={current ? estadoEfectivo(current) : null} />
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        {current?.tipo_plan === "Prepago" && (
          <Resumen
            label="Vence el"
            valor={fechaVencimiento ? formatFecha(fechaVencimiento) : "— (aún no está Activa)"}
          />
        )}
      ...
      {puedeEditar && (
        <div className="flex flex-wrap gap-3 mb-8">
          {current && estadoEfectivo(current) === "Vencida" && (
            <>
              <button
                onClick={() => setPanel("renovar")}
                className="rounded-lg border-2 px-4 py-2 text-sm font-semibold bg-white"
                style={{ borderColor: "var(--state-activa)", color: "var(--state-activa)" }}
              >
                Renovar
              </button>
              <BotonDesactivar simId={simCard.id} />
            </>
          )}
      ...
      {panel === "renovar" && (
        <PanelRenovar simId={simCard.id} onClose={() => setPanel(null)} />
      )}
...
function BotonDesactivar({ simId }: { simId: string }) {
  ...
  function desactivar() {
    ...
    startTransition(async () => {
      const res = await desactivarSim(simId, "Desactivada desde la hoja de vida.");
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        onClick={desactivar}
        disabled={isPending}
        className="rounded-lg border-2 px-4 py-2 text-sm font-semibold bg-white disabled:opacity-60"
        style={{ borderColor: "var(--state-desactivada)", color: "var(--state-desactivada)" }}
      >
...
function PanelRenovar({ simId, onClose }: { simId: string; onClose: () => void }) {
  const [fecha, setFecha] = useState(() => new Date().toISOString().slice(0, 10));
  const [error, setError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  return (
    <PanelWrapper title="Renovar SIM" onClose={onClose}>
      <div className="flex flex-col gap-3 max-w-md">
        <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
          Mantiene el mismo cliente, plan, precio y comercial — solo mueve la fecha de entrega hacia
          adelante, reiniciando el plazo desde esa fecha.
        </p>
        <label className="text-xs font-medium" style={{ color: "var(--text-secondary)" }}>Nueva fecha de entrega</label>
        ...
        <button
          ...
          style={{ background: "var(--state-activa)" }}
        >
...
function PanelNumero({ simId, actual, onClose }: { simId: string; actual: string | null; onClose: () => void }) {
  ...
  const [info, setInfo] = useState<Awaited<ReturnType<typeof consultarNumeroCorto>> | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  // Mientras se escribe, se consulta a quién pertenece hoy ese número —
  // igual que el módulo de "Reasignar números" avisa antes de aplicar.
  useEffect(() => {
    const num = numero.trim();
    if (!num) { setInfo(null); return; }
    let vigente = true;
    const timeout = setTimeout(async () => {
      const res = await consultarNumeroCorto(simId, num);
      if (vigente) setInfo(res);
    }, 400);
  ...
  function guardar() {
    setError(null);
    if (info?.estadoOrigen === "asignado" && info.iccOrigen) {
      const ok = confirm(
        `El número ${numero.trim()} está hoy en el ICC ${info.iccOrigen} (cliente: ${info.clienteOrigen ?? "sin cliente"}, estado: ${info.estadoIccOrigen ?? "sin estado"}).\n\n` +
        `Si continúas, se le quitará a esa SIM y quedará Desactivada. ¿Reasignar el número aquí?`
      );
      if (!ok) return;
    }
    startTransition(async () => {
  ...
  return (
    <PanelWrapper title="Reasignar número corto" onClose={onClose}>
      <div className="flex flex-col gap-3 max-w-md">
        ...
        />
        {info?.estadoOrigen === "asignado" && (
          <p className="text-xs" style={{ color: "var(--state-desactivada)" }}>
            Hoy pertenece al ICC {info.iccOrigen} (cliente: {info.clienteOrigen ?? "sin cliente"}, estado: {info.estadoIccOrigen}). Si sigues, esa SIM quedará Desactivada.
          </p>
        )}
        {info?.estadoOrigen === "libre" && (
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>Este número existió antes, pero hoy no tiene ninguna SIM. Se puede asignar directo.</p>
        )}
        {info?.estadoOrigen === "no_existe" && (
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>Este número es nuevo — no tiene historial. Se puede asignar directo.</p>
        )}
        {info?.estadoOrigen === "misma_sim" && (
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>Este número ya es de esta misma SIM.</p>
        )}
        {!info && (
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>
            Si el número ya está en otra SIM, se puede tomar de todas formas — esa SIM quedará Desactivada.
          </p>
        )}
        {error && <p className="text-sm" style={{ color: "var(--state-desactivada)" }}>{error}</p>}
        <button
          ...
          onClick={guardar}
          className="rounded-lg py-2 text-sm font-semibold text-white self-start px-4 disabled:opacity-60"
          style={{ background: "var(--ink-900)" }}
        >
