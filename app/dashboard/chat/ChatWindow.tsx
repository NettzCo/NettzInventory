...
export default function ChatWindow({
  ...
  const nombrePorIdTodos = new Map(todosUsuarios.map((c) => [c.id, c.full_name]));
    ...
    <div className="flex rounded-xl border bg-white overflow-hidden" style={{ borderColor: "var(--border)", height: "calc(100vh - 180px)" }}>
      ...
      {gestionAbierta ? (
        ...
        >
          ...
          <div className="px-5 py-3 border-b flex items-start justify-between gap-3 relative" style={{ borderColor: "var(--border)" }}>
            <div>
            ...
            {esGrupo && (
              <div className="relative flex-shrink-0">
                <button
                  onClick={() => setMiembrosAbiertos((v) => !v)}
                  className="text-xs font-medium rounded-lg border px-2.5 py-1.5 hover:bg-[var(--bg)] cursor-pointer whitespace-nowrap"
                  style={{ borderColor: "var(--border)", color: "var(--text-secondary)" }}
                >
                  👥 Miembros ({(miembrosPorGrupo[grupoIdActual as string] ?? []).length})
                </button>
                {miembrosAbiertos && (
                  <div
                    className="absolute right-0 top-full mt-1 rounded-lg border bg-white shadow-lg z-10 py-2"
                    style={{ borderColor: "var(--border)", minWidth: "13rem" }}
                  >
                    <p className="px-3 pb-1.5 text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-muted)" }}>
                      Miembros del grupo
                    </p>
                    {(miembrosPorGrupo[grupoIdActual as string] ?? []).length === 0 ? (
                      <p className="px-3 py-1.5 text-sm" style={{ color: "var(--text-muted)" }}>Sin miembros.</p>
                    ) : (
                      (miembrosPorGrupo[grupoIdActual as string] ?? []).map((id) => (
                        <p key={id} className="px-3 py-1.5 text-sm">
                          {nombrePorIdTodos.get(id) ?? "Usuario"} {id === currentUserId && "(tú)"}
                        </p>
                      ))
                    )}
                  </div>
