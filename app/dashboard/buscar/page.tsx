        ...
        <>
          ...
          {resultados.length > 0 && (
            <div className="flex gap-3 mb-4">
              <a href={`/dashboard/buscar/exportar?tipo=simple&q=${encodeURIComponent(q ?? "")}`} className="rounded-lg border px-4 py-2 text-sm font-medium bg-white" style={{ borderColor: "var(--border)" }}>
                {"\u2B07"} Descargar (simple)
              </a>
              <a href={`/dashboard/buscar/exportar?tipo=completa&q=${encodeURIComponent(q ?? "")}`} className="rounded-lg border px-4 py-2 text-sm font-medium bg-white" style={{ borderColor: "var(--border)" }}>
