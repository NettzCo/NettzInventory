...
export async function GET(request: NextRequest) {
  ...
  const [{ data: sims }, { data: historial }] = await Promise.all([
    supabase.from("sim_current_view").select("*"),
    supabase.from("sim_status_history").select("sim_id, changed_at").eq("estado", "Activa").order("changed_at", { ascending: true }),
  ]);

  const ultimaActivacionPorSim: Record<string, string> = {};
  for (const row of historial ?? []) ultimaActivacionPorSim[row.sim_id] = row.changed_at;

  const alertas = calcularAlertas((sims ?? []) as SimCurrentView[], ultimaActivacionPorSim, { estados, proveedor, umbralDias: umbral });
