...
import { crearPedido, editarPedido, eliminarPedido, marcarPedidoEnviado, subirComprobante, PedidoInput } from "./actions";
      ...
      <tr className="border-b last:border-0" style={{ borderColor: confirmandoEnvio ? "transparent" : "var(--border)" }}>
        ...
        <td className="px-4 py-3">
          <span
            ...
            style={{
              background: pedido.estado === "Enviado" ? "#E7F5EC" : "#FDF3E4",
              color: pedido.estado === "Enviado" ? "var(--state-activa)" : "var(--state-lista)",
            }}
          >
            {pedido.estado}
          </span>
        ...
        <td className="px-4 py-3 text-right whitespace-nowrap">
          <div className="flex flex-col gap-1 items-end">
            {pedido.estado === "Pendiente" && puedeConfirmarEnvio && (
              <button onClick={() => setConfirmandoEnvio((v) => !v)} disabled={isPending} className="text-sm font-medium" style={{ color: "var(--state-activa)" }}>
                Marcar como enviado
              </button>
            )}
            {puedeEditar && (
              <button onClick={() => setEditando(true)} disabled={isPending} className="text-sm" style={{ color: "var(--text-secondary)" }}>
                Editar
              </button>
            )}
            {puedeEditar && (
    ...
    <tr className="border-b last:border-0" style={{ borderColor: "var(--border)" }}>
      <td colSpan={9} className="px-6 py-4" style={{ background: "var(--bg)" }}>
        <div className="flex flex-wrap items-end gap-4">
          ...
          <button onClick={confirmar} disabled={isPending} className="rounded-lg px-4 py-2 text-sm font-semibold text-white disabled:opacity-60" style={{ background: "var(--state-activa)" }}>
            {isPending ? "Guardando…" : "Confirmar envío"}
          </button>
          <button onClick={onCancelar} disabled={isPending} className="rounded-lg border px-4 py-2 text-sm font-medium" style={{ borderColor: "var(--border)" }}>
            Cancelar
          </button>
        </div>
        {error && <p className="text-sm mt-2" style={{ color: "var(--state-desactivada)" }}>{error}</p>}
