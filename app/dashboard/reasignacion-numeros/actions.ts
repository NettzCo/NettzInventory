...
async function analizarPar(
  ...
  if (activo) {
    ...
    base.estadoOrigen = "asignado";
    base.simIdOrigen = activo.sim_id;
    base.iccOrigen = origen?.icc ?? null;
    base.clienteOrigen = origen?.cliente_actual ?? null;
    base.estadoIccOrigen = (origen?.estado_actual as EstadoSim | null) ?? null;

    if (base.iccOrigen === iccDestino) {
    ...
    base.estadoOrigen = "libre";
  } else {
    base.estadoOrigen = "no_existe";
  }
    ...
    try {
      ...
      if (f.estadoOrigen === "asignado" && f.simIdOrigen && f.simIdOrigen !== f.simIdDestino) {
        const { error: errorLiberar } = await supabase
          ...
          .eq("sim_id", f.simIdOrigen)
          .eq("numero_corto", f.numeroCorto)
          .is("unassigned_at", null);
        if (errorLiberar) throw new Error(`No se pudo liberar el número del ICC ${f.iccOrigen}: ${errorLiberar.message}`);

        const { error: errorDesactivar } = await supabase.from("sim_status_history").insert({
          sim_id: f.simIdOrigen,
          estado: "Desactivada",
          estado_anterior: f.estadoIccOrigen,
          changed_by: userId,
          nota: `Desactivada automáticamente: su número corto ${f.numeroCorto} se reasignó al ICC ${f.iccDestino}.`,
          bulk_operation_id: operacionId,
        });
        if (errorDesactivar) throw new Error(`Se liberó el número del ICC ${f.iccOrigen}, pero no se pudo desactivarla: ${errorDesactivar.message}`);
      }
