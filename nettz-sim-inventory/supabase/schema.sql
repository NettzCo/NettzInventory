...
select id, 'Super administrador', true, array['inventario', 'buscar', 'alertas', 'nueva', 'clientes', 'chat']
from organizations where name = 'Nettz'
on conflict (organization_id, name) do nothing;

insert into roles (organization_id, name, is_system, default_modulos)
select id, 'Comercial', false, array['inventario', 'buscar', 'alertas', 'nueva', 'clientes', 'chat']
from organizations where name = 'Nettz'
on conflict (organization_id, name) do nothing;

insert into roles (organization_id, name, is_system, default_modulos)
select id, 'Broker', false, array['inventario', 'buscar', 'alertas', 'chat']
from organizations where name = 'Nettz'
on conflict (organization_id, name) do nothing;

insert into roles (organization_id, name, is_system, default_modulos)
select id, 'Facturación', false, array['inventario', 'buscar', 'alertas', 'chat']
from organizations where name = 'Nettz'
on conflict (organization_id, name) do nothing;
...
create table if not exists profiles (
  ...
  modulos text[] not null default array['inventario', 'buscar', 'alertas', 'chat'],
...
-- quien tenga el rol "Super administrador" (is_system) puede ver y
-- administrar todas — es el mismo permiso que abre "Configuración".
create policy "read own organization" on organizations for select
  using (id = current_org_id() or current_role_is_system());
create policy "super_admin manage organizations" on organizations for insert
  with check (current_role_is_system());
create policy "super_admin update organizations" on organizations for update
  using (current_role_is_system());

-- Perfiles: se leen dentro de la misma organización (o cualquier super
-- administrador puede leer cualquiera)
create policy "read profiles same org" on profiles for select
  using (organization_id = current_org_id() or current_role_is_system());
...
create policy "write sim_cards" on sim_cards for insert with check (
  organization_id = current_org_id() and tiene_modulo('nueva')
);
...
create policy "write short_numbers ins" on sim_short_numbers for insert with check (
  exists (select 1 from sim_cards sc where sc.id = sim_id and sc.organization_id = current_org_id()) and tiene_modulo('nueva')
);
...
create policy "super_admin upload logos" on storage.objects for insert
  with check (bucket_id = 'org-logos' and current_role_is_system());
create policy "super_admin update logos" on storage.objects for update
  using (bucket_id = 'org-logos' and current_role_is_system());
