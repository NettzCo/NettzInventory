...
export async function GET() {
  ...
  const notas: Record<string, string> = {
    ...
    fecha_entrega: "Obligatorio. Formato AAAA-MM-DD (ej: 2026-08-16).",
    comercial_correo: "Obligatorio. Elige de la lista desplegable (usuarios activos con rol comercial o super administrador).",
    broker_correo: "Opcional. Elige de la lista desplegable si aplica.",
  };
  ...
  function agregarValidacion(key: string, rangoListaKey: string) {
    ...
    for (let row = 2; row <= 500; row++) {
      sheet.getCell(row, col).dataValidation = {
        type: "list",
        allowBlank: key === "apn" || key === "broker_correo",
