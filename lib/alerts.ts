...
import { sumarMeses, hoyIso, diferenciaDiasIso } from "./ui";
 ...
 *  `fechaAniversario` ya viene como "YYYY-MM-DD" (ver sumarMeses). */
export function llaveAlerta(simId: string, fechaAniversario: string): string {
  return `${simId}|${fechaAniversario}`;
}
 ...
 *
 * Todo el cálculo de fechas es aritmética pura de calendario (sumarMeses,
 * diferenciaDiasIso) — nunca un objeto Date con hora, así el resultado
 * nunca se corre un día según la zona horaria de quien lo ejecute.
 *
...
export function calcularAlertas(
  ...
  for (const sim of sims) {
    ...
    const fechaAniversario = sumarMeses(sim.fecha_entrega, meses);
    const diasRestantes = diferenciaDiasIso(fechaAniversario, hoy);
